...
mongoose.connect("mongodb://localhost/voicewithme-dev", function(error) {
  if (error)
    console.log(error);
...
// get parents by user id
app.get('/api/parent/:userId', async function(req, res, next) {
  var parents = await Parent.find({userId: req.params.userId}).sort({createdAt: -1}).limit(30);
  res.send(parents);
});
